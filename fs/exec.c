...
static unsigned long copy_strings(int argc, char **argv, unsigned long *page,
  ...
  while (argc-- > 0) {
    ...
    if (from_kmem == 1) set_fs(new_fs);
    if (!(tmp = (char *)get_fs_long(((unsigned long *)argv) + argc)))
      panic("argc is wrong");
    if (from_kmem == 1) set_fs(old_fs);
	...
	if (!(pag = (char *)page[p / PAGE_SIZE])) {
	  ...
	  if (!(pag = (char *)page[p / PAGE_SIZE])) return 0;
	}
...
int do_execve(unsigned long *eip, long tmp, char *filename, char **argv,
  ...
  unsigned long p =
      PAGE_SIZE * MAX_ARG_PAGES - 4;  // p指向参数和环境空间的最后部
...
restart_interp:
  ...
  for (i = 0; i < NR_OPEN; i++)
    if ((current->close_on_exec >> i) & 1) sys_close(i);
...
exec_error2:
  iput(inode);
exec_error1:
  for (i = 0; i < MAX_ARG_PAGES; i++) free_page(page[i]);
  return (retval);
}
